...
def make_hits_non_gaussian_gamma(params):
    xs=[]
    ys=[]
    zs =[]
    ...
    for r0 in np.linspace(min_r0,max_r0,nlayers):
        ...
        noise_x = gamma(shape_param, scale=3*sigma)
        noise_y = gamma(shape_param, scale=3*sigma)
        noise_z = gamma(shape_param, scale=3*sigma)
        
        xs.append(x0+noise_x)
        ys.append(y0+noise_y)
        zs.append(z0+noise_z)
...
def gen_tracks(n=1):
    ...
    for i in range(n):
        ...
        xs,ys,zs = make_hits_non_gaussian_gamma(params)
...
f=open("non_gaussian_tracks\\tracks_100k_skewed_large_test.txt","w")
for track in tracks:
    params = track[0]
    xs = track[1]
    ys = track[2]
