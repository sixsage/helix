...
ENCODER_RESULTS_PATH = 'variance_predict\\gaussian\\weighted_no_trig_results2\\'
DECODER_RESULTS_PATH = 'variance_predict\\gaussian\\weighted_no_trig_results2\\'
...
D0_WEIGHT, PHI0_WEIGHT, PT_WEIGHT, DZ_WEIGHT, TANL_WEIGHT = [0.01955948, 0.48037714, 0.00740729, 0.0124219, 0.4802342]
...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        target = self.original_targets_rescaled[idx]
...
class WeightedMSE(nn.Module):
    ...
    def forward(self, predictions, target, variance):
        ...
        pred_d0, pred_phi0, pred_pt, pred_dz, pred_tanl = predictions[:, 0], predictions[:, 1], predictions[:, 2], predictions[:, 3], predictions[:, 4]
        target_d0, target_phi0, target_pt, target_dz, target_tanl = target[:, 0], target[:, 1], target[:, 2], target[:, 3], target[:, 4]

        d0_mse = ((target_d0 - pred_d0) ** 2) * D0_WEIGHT
        phi0_mse = ((target_phi0 - pred_phi0) ** 2) * PHI0_WEIGHT
        pt_mse = ((target_pt - pred_pt) ** 2) * PT_WEIGHT
        dz_mse = ((target_dz - pred_dz) ** 2) * DZ_WEIGHT
        tanl_mse = ((target_tanl - pred_tanl) ** 2) * TANL_WEIGHT

        mse_term = torch.stack((d0_mse, phi0_mse, pt_mse, dz_mse, tanl_mse), dim=1)
