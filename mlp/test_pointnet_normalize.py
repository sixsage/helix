...
RESULTS_PATH = 'mlp\\1m_pointnet_normalize\\'
MODEL_PATH = 'pointnet_epoch_60.pth'
...
if __name__ == '__main__':
    ...
    dataset = PointNet_Dataset(path=DATASET_PATH)
    ...
    with torch.no_grad():
        for inputs, targets in val_dataloader:
            ...
            outputs = torch.Tensor.cpu(model(inputs)) # conversion to numpy array needed
            outputs = dataset.scaler.inverse_transform(outputs)
            outputs = torch.tensor(outputs).to(device) # back to tensor
